(function() {
	...
	TXLIVE_PRIVATE.isNotTextual = /^(&nbsp;|\s|\d|[$-\/:-?{-~!"^_`\[\]]|[@!#\u20ac\u00a3\u0192\u20b9\u00a5])*$/;
	...
	var SIDEBAR_URL_TRIGGER = "transifexliveftw";
	var console = window.console;
	TXLIVE.lib_version = LIB_VERSION;
	TXLIVE.raven_extra = { lib_version: TXLIVE.lib_version, url: TXLIVE.editor && TXLIVE.editor.url ? TXLIVE.editor.url : document.location.href, iframe: TXLIVE.editor ? true : false };
	TXLIVE.settings = {
		autocollected: false,
		has_storage: false
	};
	...
	function setSettings(options, override) {
		if (!options)return;
		if (override)for (var k in options)TXLIVE.settings[k] = options[k];
		else for (var k in options)if (window.liveSettings[k] === undefined)TXLIVE.settings[k] = options[k];
		TXLIVE.settings.autocollect = TXLIVE.settings.autocollect | 0;
		TXLIVE.settings.prerender = TXLIVE.settings.prerender | 0;
		TXLIVE.settings.dynamic = TXLIVE.settings.dynamic | 0;
		TXLIVE.settings.staging = TXLIVE.settings.staging | 0;
		...
		if (TXLIVE.settings.detectlang && typeof TXLIVE.settings.detectlang === "string")TXLIVE.settings.detectlang = TXLIVE.settings.detectlang | 0;
		if (TXLIVE.settings.parse_attr && TXLIVE.settings.parse_attr.length)for (var i = 0; i < TXLIVE.settings.parse_attr.length; ++i)TXLIVE.settings.parse_attr[i] = TXLIVE.settings.parse_attr[i].toLowerCase();
		if (TXLIVE.settings.ignore_tags && TXLIVE.settings.ignore_tags.length)for (var i = 0; i < TXLIVE.settings.ignore_tags.length; ++i)TXLIVE.settings.ignore_tags[i] = (TXLIVE.settings.ignore_tags[i] || "").toUpperCase();
		if (TXLIVE.settings.ignore_class && TXLIVE.settings.ignore_class.length)for (var i = 0; i < TXLIVE.settings.ignore_class.length; ++i)TXLIVE.settings.ignore_class[i] = (TXLIVE.settings.ignore_class[i] || "").toLowerCase()
	}
	...
	if (!window.Raven) {
		!function(a,
			...
			function f(a) { return"undefined" == typeof a }
			...
			function k(a, b) {
				var c, d;
				if (f(a.length))for (c in a)j(a, c) && b.call(null, c, a[c]);
				else if (d = a.length)for (c = 0; d > c; c++)b.call(null, c, a[c])
			}

			function l() {
				J = "?sentry_version=4&sentry_client=raven-js/" +
					O.VERSION + "&sentry_key=" + H
			}
			...
			function m(a, b) {
				...
				a.stack && a.stack.length && k(a.stack, function(a, b) {
					var c = n(b);
					c && d.push(c)
				}), c("handle", { stackInfo: a, options: b }), p(a.name, a.message, a.url, a.lineno, d, b)
			}

			function n(a) {
				if (a.url) {
					var b, c = { filename: a.url, lineno: a.line, colno: a.column, "function": a.func || "?" }, d = o(a);
					if (d) {
			...
			function p(a, b, c, d, e, f) {
				var g, h;
				b += "", ("Error" !== a || b) && (M.ignoreErrors.test(b) || (e && e.length ? (c = e[0].filename || c, e.reverse(), g = { frames: e }) : c && (g = { frames: [{ filename: c, lineno: d, in_app: !0 }] }), b = r(b, 100), M.ignoreUrls && M.ignoreUrls.test(c) || (!M.whitelistUrls || M.whitelistUrls.test(c)) &&
				(h = d ? b + " at " + d : b, t(q({ exception: { type: a, value: b }, stacktrace: g, culprit: c, message: h }, f)))))
			}

			function q(a, b) { return b ? (k(b, function(b, c) { a[b] = c }), a) : a }

			function r(a, b) { return a.length <= b ? a : a.substr(0, b) + "\u00e2\u20ac\u00a6" }

			function s() {
				var a = { url: document.location.href, headers: { "User-Agent": navigator.userAgent } };
				return document.referrer && (a.headers.Referer = document.referrer), a
			}

			function t(a) {
				v() && (a = q({ project: I, logger: M.logger, site: M.site, platform: "javascript", request: s() }, a), a.tags = q(M.tags, a.tags),
					a.extra = q(M.extra, a.extra), i(a.tags) && delete a.tags, i(a.extra) && delete a.extra, G && (a.user = G), g(M.dataCallback) && (a = M.dataCallback(a)), (!g(M.shouldSendCallback) || M.shouldSendCallback(a)) && (E = a.event_id || (a.event_id = x()), u(a)))
			}

			function u(a) {
				var b = new Image, d = F + J + "&sentry_data=" + encodeURIComponent(JSON.stringify(a));
				b.onload = function() { c("success", { data: a, src: d }) }, b.onerror = b.onabort = function() { c("failure", { data: a, src: d }) }, b.src = d
			}

			function v() {
				return L ? F ? !0 : (y("error", "Error: Raven has not been configured."),
					!1) : !1
			}
			...
			function y(b, c) { a.console && console[b] && O.debug && console[b](c) }
			...
			A.wrap = function(a) {
				...
				function c(a) { h(), o.push(a) }

				function d(a) { for (var b = o.length - 1; b >= 0; --b)o[b] === a && o.splice(b, 1) }
				...
				function g(a, b, c, d, e) {
					var g = null;
					if (r)
						A.computeStackTrace.augmentStackTraceWithInitialElement(r,
							b, c, a), k();
					else if (e)g = A.computeStackTrace(e), f(g, !0);
					else {
						var h = { url: b, line: c, column: d };
						h.func = A.computeStackTrace.guessFunctionName(h.url, h.line), h.context = A.computeStackTrace.gatherContext(h.url, h.line), g = { message: a, url: document.location.href, stack: [h] }, f(g, !0)
					}
					return m ? m.apply(this, arguments) : !1
				}

				function h() { n || (m = a.onerror, a.onerror = g, n = !0) }

				function i() { n && (a.onerror = m, n = !1, m = b) }
				...
				function l(b, c) {
					...
					if (r = e, q = b, p = d, a.setTimeout(function() { q === b && k() }, e.incomplete ? 2E3 : 0), c !== !1)throw b;
				}
				...
				function i(a) { return g(a).replace("<", "(?:<|&lt;)").replace(">", "(?:>|&gt;)").replace("&", "(?:&|&amp;)").replace('"', '(?:"|&quot;)').replace(/\s+/g, "\\s+") }
				...
				function n(a) {
					...
					for (var b,
						c,
						g = /^\s*at (?:((?:\[object object\])?\S+(?: \[as \S+\])?) )?\(?((?:file|https?|chrome-extension):.*?):(\d+)(?::(\d+))?\)?\s*$/i,
						h = /^\s*(\S*)(?:\((.*?)\))?@((?:file|https?|chrome).*?):(\d+)(?::(\d+))?\s*$/i,
						i = a.stack.split("\n"),
						j = [],
						k = /^(.*) is undefined$/.exec(a.message),
						m = 0,
						n = i.length; n > m; ++m) {
						if (b = h.exec(i[m]))c = { url: b[3], func: b[1] || C, args: b[2] ? b[2].split(",") : "", line: +b[4], column: b[5] ? +b[5] : null };
						else {
							if (!(b = g.exec(i[m])))continue;
							c = { url: b[2], func: b[1] || C, line: +b[3], column: b[4] ? +b[4] : null }
						}
						!c.func && c.line && (c.func = d(c.url, c.line)), c.line && (c.context = e(c.url, c.line)), j.push(c)
					}
					return j.length ? (j[0].line && !j[0].column &&
						k ? j[0].column = l(k[1], j[0].url, j[0].line) : j[0].column || f(a.columnNumber) || (j[0].column = a.columnNumber + 1), { name: a.name, message: a.message, url: document.location.href, stack: j }) : null
				}

				function o(a) {
					for (var b, c = a.stacktrace, f = / line (\d+), column (\d+) in (?:<anonymous function: ([^>]+)>|([^\)]+))\((.*)\) in (.*):\s*$/i, g = c.split("\n"), h = [], i = 0, j = g.length; j > i; i += 2)
						if (b = f.exec(g[i])) {
							var k = { line: +b[1], column: +b[2], func: b[3] || b[4], args: b[5] ? b[5].split(",") : [], url: b[6] };
							if (!k.func && k.line && (k.func = d(k.url, k.line)),
								k.line)
								try {
				...
				function p(b) {
					...
					for (h = 2, l = f.length; l > h; h += 2) {
						var t = null;
						if (g = n.exec(f[h]))t = { url: g[2], func: g[3], line: +g[1] };
							...
							m = k(z, [x]), t = { url: x, line: m ? m.line : y, func: "" }
						}
						if (t) {
				...
				function r(a, b) {
					for (var c, e, f, g = /function\s+([_$a-zA-Z\xA0-\uFFFF][_$a-zA-Z0-9\xA0-\uFFFF]*)?\s*\(/i, h = [], i = {}, j = !1, k = r.caller; k && !j; k = k.caller)
						if (k !== s && k !== A.report) {
							if (e = { url: null, func: C, line: null, column: null }, k.name ? e.func = k.name : (c = g.exec(k.toString())) && (e.func = c[1]), f = m(k)) {
								e.url = f.url, e.line = f.line, e.func === C && (e.func =
									d(e.url, e.line));
					...
					b && h.splice(0, b);
					var o = { name: a.name, message: a.message, url: document.location.href, stack: h };
			...
			var D,
				...
				M = { logger: "javascript", ignoreErrors: [], ignoreUrls: [], whitelistUrls: [], includePaths: [], collectWindowErrors: !0, tags: {}, extra: {} },
				N = !1,
				O = {
					VERSION: "1.1.16",
					debug: !0,
					...
					config: function(a, b) {
						...
						return b && k(b, function(a, b) { M[a] = b }), M.ignoreErrors.push("Script error."), M.ignoreErrors.push("Script error"), M.ignoreErrors.push("Javascript error: Script error on line 0"), M.ignoreErrors.push("Javascript error: Script error. on line 0"), M.ignoreErrors = w(M.ignoreErrors), M.ignoreUrls = M.ignoreUrls.length ? w(M.ignoreUrls) : !1, M.whitelistUrls =
							M.whitelistUrls.length ? w(M.whitelistUrls) : !1, M.includePaths = w(M.includePaths), H = c.user, I = c.path.substr(d + 1), F = "//" + c.host + (c.port ? ":" + c.port : "") + "/" + f + "api/" + I + "/store/", c.protocol && (F = c.protocol + ":" + F), M.fetchContext && (A.remoteFetching = !0), M.linesOfContext && (A.linesOfContext = M.linesOfContext), A.collectWindowErrors = !!M.collectWindowErrors, l(), O
					},
					install: function() { return v() && !N && (A.report.subscribe(m), N = !0), O },
					context: function(a, c, d) { return g(a) && (d = c || [], c = a, a = b), O.wrap(a, c).apply(this, d) },
					wrap: function(a,
						c) {
						function d() {
					...
					uninstall: function() { return A.report.uninstall(), N = !1, O },
					...
					captureMessage: function(a, b) { return t(q({ message: a + "" }, b)), O },
					setUserContext: function(a) { return G = a, O },
					setExtraContext: function(a) { return M.extra = a || {}, O },
					setTagsContext: function(a) { return M.tags = a || {}, O },
					lastException: function() { return D },
					lastEventId: function() { return E }
				};
			O.setUser = O.setUserContext;
			var P = "source protocol user pass host port path".split(" "), Q = /^(?:(\w+):)?\/\/(\w+)(:\w+)?@([\w\.-]+)(?::(\d+))?(\/.*)/;
			d.prototype = new Error, d.prototype.constructor =
				d, z(), a.Raven = O, "function" == typeof define && define.amd && define("raven", [], function() { return O })
		}(this);
		Raven.config("https://7569be2778c0403186b7f224b35b68ed@app.getsentry.com/17391", {
			collectWindowErrors: false,
			whitelistUrls: [],
			ignoreErrors: [
				"top.GLOBALS", "originalCreateNotification", "canvas.contentDocument", "MyApp_RemoveAllHighlights", "http://tt.epicplay.com", "Can't find variable: ZiteReader", "jigsaw is not defined", "ComboSearch is not defined", "http://loading.retry.widdit.com/", "atomicFindClose",
				"fb_xd_fragment", "bmi_SafeAddOnload", "EBCallBackMessageReceived", "conduitPage"
			],
			...
			shouldSendCallback: function(data) {
				if (data && data.message &&
					data.message.length)return data.message.indexOf("TXLive:") >= 0 || data.message.indexOf("TXBridge:") >= 0;
				return true
			}
		}).install()
	}
	...
	(function() {
		try {
			...
			css += "  to {opacity:inherit;}\n";
			css += "}\n";
			css += ".txlive-dynamic {animation: txlivenodeinserted 0.001s; -webkit-animation: txlivenodeinserted 0.001s;}";
	...
	function showDom() {
		try {
			...
			if (document.getElementsByClassName)
				el =
					document.getElementsByClassName("txlive");
	...
	(function() {
		...
		TXLIVE_PRIVATE.JSON.__Date__toJSON = function() { return isFinite(this.valueOf()) ? this.getUTCFullYear() + "-" + f(this.getUTCMonth() + 1) + "-" + f(this.getUTCDate()) + "T" + f(this.getUTCHours()) + ":" + f(this.getUTCMinutes()) + ":" + f(this.getUTCSeconds()) + "." + f_msec(this.getUTCMilliseconds()) + "Z" : null };
		var cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
	...
	function callFunctionArray(fcallarray, param) {
		...
		for (var i = 0; i < fcallarray.length; ++i)
			try {
				fcallarray[i](param)
			} catch (err) {
				TXLIVE.logger.error(err)
	...
	if (!String.prototype.trim)
		String.prototype.trim =
			function() { return TXLIVE_PRIVATE.trim(this) };
	TXLIVE.loadScript = function(url, callback) {
		if (url.indexOf("//") == 0)
	...
	TXLIVE_PRIVATE.getBrowserLocale = function() {
		...
		if (navigator.userLanguage) {
			var code =
				TXLIVE.normalizeLangCode(navigator.userLanguage);
			if (TXLIVE.hasLanguageCode(code))return code
		}
		...
		if (navigator.userLanguage) {
			var code = TXLIVE.matchLanguageCode(navigator.userLanguage);
			if (code)return code
		}
		if (navigator.language) {
			var code =
				TXLIVE.matchLanguageCode(navigator.language);
			if (code)return code
		}
		return null
	};
	...
	TXLIVE_PRIVATE.getElementsByClassName =
	...
	TXLIVE_PRIVATE.bindClick = function(node, fn) {
		if (node.addEventListener)node.addEventListener("click", function() { fn && fn(node) });
		else node.attachEvent("onclick", function() { fn && fn(node) })
	};
	...
	TXLIVE_PRIVATE.addClass = function(node, cls) { if (!TXLIVE_PRIVATE.hasClass(node, cls) && node.className != undefined && node.className.baseVal == undefined)node.className = (node.className || "") + " " + cls };
	TXLIVE_PRIVATE.removeNodeByID = function(id) {
		var element = document.getElementById(id);
		if (element && element.parentNode)element.parentNode.removeChild(element)
	};
	TXLIVE_PRIVATE.escapeLanguageCode = function(code) { return code.replace(/-/g, "_").replace(/[.@]/g, "__") };
	...
	TXLIVE_PRIVATE.getWindowPath = function() { return TXLIVE_PRIVATE.constructPath(window.location.pathname, window.location.search) };
	TXLIVE_PRIVATE.getWindowHost = function() { return document.location.hostname };
	TXLIVE.doCORSRequest = function(url, method, data, callback, errback) {
		...
		if (window.XMLHttpRequest) {
			...
			req.open(method, url);
			if (errback)req.onerror = errback;
			req.onload = function() { callback && callback(req.responseText) };
			req.send(data)
		} else errback && errback(new Error("doCORSRequest: CORS not supported by browser"))
	};
	TXLIVE_PRIVATE.md5 = function() {
		var rotateLeft = function(lValue,
			iShiftBits) { return lValue << iShiftBits | lValue >>> 32 - iShiftBits };
	...
	TXLIVE_PRIVATE.addPicker = function(picker) {
		...
		try {
			...
			if (el)TXLIVE_PRIVATE.bindClick(el, function() { is_picker_open() ? close_picker() : open_picker() });
			el = document.getElementById("tx-live-lang-picker");
			if (el) {
				el = el.getElementsByTagName("li") || [];
				for (var i = 0; i < el.length; ++i)
					TXLIVE_PRIVATE.bindClick(el[i],
						pick_lang)
			}
		} catch (err) {
			TXLIVE.logger.error(err)
	...
	var SKIP_TAGS = { SCRIPT: true, STYLE: true, LINK: true, IFRAME: true, NOSCRIPT: true, CANVAS: true, SVG: true, svg: true, AUDIO: true, VIDEO: true, CODE: true, TIME: true, VAR: true, KBD: true };
	var SKIP_CLASS =
	{ "notranslate": true, "txlive-dynamic-processed": true, "txlive-meta": true, "facebook_container": true, "twitter_container": true };
	...
	function isSkipClass(className) {
		...
		var cls_array =
			className.toLowerCase().match(/\S+/g);
	...
	function GET_FRAGMENT(item) {
		var node =
			item.snode_before ? item.snode_before.nextSibling : item.snode_after.parentNode.firstChild;
		var tmp = document.createElement("div");
		while (node && node != item.snode_after) {
	...
	function PARSE_ARGS(node, args) {
		if (node) {
			do
				if (node.nodeType === 1)
					if (isSkipTag(node.tagName) || isSkipClass(node.className)) {
						var outerhtml = node.outerHTML;
						if (outerhtml)args.push({ type: "VAR", html: outerhtml })
	...
	function PARSE_VARIABLES(text, node, args, options) {
		PARSE_ARGS(node.firstChild,
			args);
		for (var i = 0; i < args.length; ++i)if (args[i].type === "VAR")text = text.replace(args[i].html, "{{" + i + "}}");
	...
	function CRAWL_FRAGMENT(snode_before, snode_after, tags, options) {
		...
		try {
			if (snode_before)
				snode_before.txbefore_detected =
					true;
		...
		segment.elements.push({ segment: segment, snode_before: snode_before, snode_after: snode_after, set: SET_FRAGMENT, get: GET_FRAGMENT, block_args: args, head: raw_text.indexOf(" ") == 0 ? " " : "", tail: raw_text.indexOf(" ", raw_text.length - 1) >= 0 ? " " : "" });
	...
	function CRAWL_BLOCK(node, tags, options) {
		...
		if (tags.length)mergeArrays(segment.tags, tags);
		segment.elements.push({ segment: segment, node: node, pnode: node, set: SET_BLOCK, get: GET_BLOCK, block_args: args, head: raw_text.indexOf(" ") == 0 ? " " : "", tail: raw_text.indexOf(" ", raw_text.length - 1) >= 0 ? " " : "" });
		if (NEW_SEGMENTS_LIST && !NEW_SEGMENTS_DICT[key]) {
			NEW_SEGMENTS_LIST.push(segment);
			NEW_SEGMENTS_DICT[key] = segment
		}
	...
	function CRAWL_ATTR(node, attr, tags) {
		...
		if (text && text.length && !TXLIVE_PRIVATE.isNotTextual.test(text)) {
			...
			if (tags.length)mergeArrays(segment.tags, tags);
			segment.elements.push({ segment: segment, node: node, pnode: node.parentNode, set: SET_ATTR, get: GET_ATTR, attribute: attr, head: raw_text.indexOf(" ") == 0 ? " " : "", tail: raw_text.indexOf(" ", raw_text.length - 1) >= 0 ? " " : "" });
			if (NEW_SEGMENTS_LIST && !NEW_SEGMENTS_DICT[key]) {
				NEW_SEGMENTS_LIST.push(segment);
				NEW_SEGMENTS_DICT[key] = segment
			}
	...
	function PARSE_DOM(node, parent_tags, options) {
		if (node)
			if (!isSkipTag(node.tagName))
				if (node.nodeType ===
					3)PARSER_OPTIONS.isunset(options, PARSER_OPTIONS.DO_NOT_COLLECT) && CRAWL_TEXT(node, parent_tags);
							...
							if (isBlockElement(childnode)) {
								...
								if (!nextnode || nextnode && isBlockElement(nextnode)) {
									...
									if (!has_text)
										...
										_collect && CRAWL_BLOCK(node, tags, _options);
										childnode = null
									}
								}
							}
					}
					if (NEW_SEGMENTS_LIST && TXLIVE_PRIVATE.hasClass(node, "txlive-dynamic"))TXLIVE_PRIVATE.addClass(node, "txlive-dynamic-processed")
				}
	}

	function CLEAR_FLAGS_DOM(node) {
		if (!node)return;
		try {
	...
	function autoCollect(custom_segments) {
		if (TXLIVE_PRIVATE.extractDomain(TXLIVE.settings.domain) !==
			TXLIVE_PRIVATE.extractDomain(TXLIVE_PRIVATE.getWindowLocation().href))return;
		if (!TXLIVE.settings.has_storage || !TXLIVE.settings.autocollect || TXLIVE.settings.autocollected && !custom_segments)return;
		TXLIVE.settings.autocollected = !custom_segments;
		try {
			var timestamp = Date.now();
			var vault = TXLIVE_PRIVATE.storage_get("txlive:vault") || {};
			var segments = custom_segments || TXLIVE.segments;
			var new_entities = {};
			var refresh_ts = [];
			var found = false;
			var published_keys = LAST_TRANSLATION_JSON || {};
			for (var key in segments) {
				var s =
					segments[key];
				if (!s.source_string)continue;
				var collected = key in published_keys;
				var vault_entry = vault[key];
				var to_autocollect = false;
				if (!vault_entry) {
					to_autocollect = true;
					vault[key] = { string: s.source_string, tags: s.tags, ts: timestamp }
	...
	var mutation_nodes = [];
	var observer = null;
	var mutators_initialized = false;
	var mutators_on = false;

	function __proc_mutations__() {
		timer_mutation = null;
		var node_list = [];
		for (var i = 0; i < mutation_nodes.length; ++i)if (IS_IN_DOCUMENT(mutation_nodes[i]))node_list.push(mutation_nodes[i]);
		mutation_nodes = [];
	...
	function __observer_inserted_cb(mutations) {
		mutations.forEach(function(mutation) { for (var i = 0; i < mutation.addedNodes.length; ++i)mutation_nodes.push(mutation.addedNodes[i]) });
	...
	TXLIVE.normalizeLangCode = function(langcode) {
		...
		if (langcode.indexOf("_") >= 0) {
			...
			var lang = tokens[0].toLowerCase();
			var country = tokens[1].toUpperCase();
			langcode = lang + "_" + country
		} else langcode = langcode.toLowerCase();
		if (extra)langcode += "@" + extra;
		return langcode
	};
	TXLIVE.hasTargetLanguage =
		function(langcode) {
	...
	TXLIVE.translateTo = function(langcode) {
		try {
			...
			if (TXLIVE.selected_lang === langcode)
				if (languages.source.code === langcode && !LAST_TRANSLATION_LANG || LAST_TRANSLATION_LANG && LAST_TRANSLATION_JSON && LAST_TRANSLATION_LANG ===
					langcode) {
					__ready__();
					return
				}
	...
	function setLanguages(languages) {
		try {
			...
			if (isString(languages.translation))
				try {
	...
	TXLIVE.detectLanguage = function() {
		try {
			if (TXLIVE.settings.detectlang && isFunction(TXLIVE.settings.detectlang))return TXLIVE.settings.detectlang();
			if (!TXLIVE.settings.detectlang)return TXLIVE_PRIVATE.storage_get("txlive:selectedlang");
	...
	TXLIVE.hasLanguageCode = function(langcode) {
		try {
			...
			var d = TXLIVE.denormalized_languages;
			for (var i = 0; i < d.length; ++i)if (d[i].code === langcode)return true;
			return false
		} catch (err) {
			TXLIVE.logger.error(err)
		}
	...
	TXLIVE.onFetchLanguages = function(fcall) {
		...
		if (TXLIVE.languages &&
			fcall)callFunctionArray([fcall], TXLIVE.denormalized_languages)
	};
	...
	TXLIVE.unBind = function(fcall) {
		...
		removeFromFunctionArray(this.__ontranslatepage, fcall);
		removeFromFunctionArray(this.__onbeforetranslate,
			fcall);
	...
	TXLIVE.translateNodes = function(node_array) {
		...
		for (var i = 0; i < node_array.length; ++i)
			try {
				if (!IS_SKIP_PARENT(node_array[i]))PARSE_DOM(node_array[i], [], PARSER_OPTIONS.DEFAULT)
			} catch (err) {
				TXLIVE.logger.error(err)
			}
	...
	function translate_text_params(text, params) {
		if (params)for (key in params)text = text.replace(new RegExp("\\{" + key + "\\}", "g"), params[key]);
		return text
	}
	...
	TXLIVE.translateText = function(text, params) {
		...
		if (LAST_TRANSLATION_JSON && LAST_TRANSLATION_JSON[key])
			return translate_text_params(LAST_TRANSLATION_JSON[key],
				params);
		var segments = TXLIVE.segments;
		var segment_list = TXLIVE.segment_list;
		var segment = segments[key];
		if (!segment) {
			var segment = { key: key, source_string: text, tags: ["api"], elements: [] };
			segments[key] = segment;
			segment_list.push(segment);
	...
	TXLIVE.segmentHTML = function(html, as_key_value_json) {
		...
		var json = {};
		if (as_key_value_json)for (var key in segments)json[key] = segments[key].source_string;
		else
			for (var key in segments) {
				var segment =
					segments[key];
	...
	function __ready__() {
		...
		if (document && document.addEventListener) {
			var insertListener = function(event) { if (event.animationName == "txlivenodeinserted")TXLIVE.translateNode(event.target) };
	...
	TXLIVE_SIDEBAR.getLocationParams = function() {
		...
		for (var i = 0; i < params_array.length; i++) {
			...
			if (param_string.indexOf("=") != -1) {
				...
				return_value[param_string] =
					null
		}
		return return_value
	};
	TXLIVE_SIDEBAR.load = function() {
		if (TXLIVE_SIDEBAR.__loaded)return;
		TXLIVE_SIDEBAR.__loaded = true;
		TXLIVE.doCORSRequest(TXLIVE.settings.sidebar_base_url + "/_/live/sidebar/", "GET", "", function(html) {
			...
			for (var i = scripts.length - 1; i >= 0; --i)if (scripts[i].src)scripts[i].parentNode.removeChild(scripts[i]);
			...
			function consume_scripts() {
				var url = urls.shift();
				url && TXLIVE.loadScript(url, consume_scripts)
			}
	...
	function __loadsidebar__() {
		if (!TXLIVE.settings.api_key)return;
	...
	function __traverseready__() {
		...
		try {
			...
			if (TXLIVE.settings.ignore_class && TXLIVE.settings.ignore_class.length)
				for (var i = 0; i < TXLIVE.settings.ignore_class.length; ++i) {
		...
		try {
			...
			PARSE_DOM(document.body ||
				document.getElementsByTagName("body")[0], [], PARSER_OPTIONS.DEFAULT)
		} catch (err) {
			TXLIVE.logger.error(err)
		}
		try {
	...
	function __reload__() {
		...
		CLEAR_FLAGS_DOM(document.body || document.getElementsByTagName("body")[0]);
		__traverseready__();
	...
	function __init__() {
		...
		if (TXLIVE.settings.api_key)Raven.setUser({ id: TXLIVE.settings.api_key });
		else if (TXLIVE.user)Raven.setUser({ id: TXLIVE.user.username, email: TXLIVE.user.email });
		TXLIVE.languages_url = TXLIVE.settings.cdn || "//cdn.transifex.com/";
		TXLIVE.autocollect_url = TXLIVE.settings.autocollect_url ||
			"//clsrv.transifex.com";
		TXLIVE.staging = TXLIVE.settings.staging ? true : false;
		...
		if (TXLIVE.settings.api_key && !TXLIVE.live_noop) {
			...
			var previous_settings = TXLIVE_PRIVATE.storage_get("txlive:settings");
			var previous_langs = TXLIVE_PRIVATE.storage_get("txlive:languages");
			...
			window.transifex_settings = function(settings) {
				...
				if (previous_settings && previous_langs && settings && TXLIVE.traverse_ready)
					try {
			...
			var base_url = TXLIVE.languages_url + TXLIVE.settings.api_key + "/" + TXLIVE.version;
			if (TXLIVE.staging)base_url += "/{}.staging.jsonp";
			else base_url += "/{}.jsonp";
			...
			TXLIVE.loadScript(base_url.replace("{}", "settings"), function() {
				__traverseready__();
				TXLIVE.loadScript(base_url.replace("{}", "languages"), function() {
					setTimeout(function() {
						if (!langs_loaded) {
							__ready__();
							callFunctionArray(TXLIVE.__onerror,
								"[ERR3] Cannot load languages url: " + base_url.replace("{}", "languages"))
						}
					}, 100)
				})
			})
